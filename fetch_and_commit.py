...
MAX_WORKERS = 20  # Уменьшено для стабильности
ICON_DOWNLOAD_WORKERS = 10  # Отдельный лимит для иконок
REQUEST_TIMEOUT = 45  # Увеличен timeout для больших файлов
ICON_TIMEOUT = 15  # Короткий timeout для иконок
MAX_ICON_DOWNLOAD_TIME = 1200  # Максимум 20 минут на загрузку всех иконок
    ...
    try:
        ...
        with requests.get(url, stream=True, timeout=REQUEST_TIMEOUT) as r:
            ...
            with open(temp_path, 'wb') as f:
                ...
                for chunk in r.iter_content(32 * 1024):  # Увеличен размер chunk
...
def download_icon_batch(session, batch_items):
    ...
    for url, save_path in batch_items:
        ...
        try:
            ...
            # Игнорируем ошибки отдельных иконок
            continue
    return successful
...
def get_icon_signature_fast(file_path):
    ...
    try:
        ...
        with open_func(file_path, 'rb') as f:
            # Используем более быстрый парсинг с ограничением
            context = etree.iterparse(f, tag='icon', events=('end',))
            count = 0
            for _, element in context:
                if count > 10000:  # Ограничиваем количество для больших файлов
                    break
...
def perform_full_update(download_results):
    ...
    # Оптимизированная загрузка иконок
    ...
    if urls_to_download:
        # Разбиваем на пакеты для контроля прогресса
...
def process_epg_file(file_path, group_map, icon_pool, owner, repo_name, entry):
    ...
    try:
        ...
        if changes_made > 0:
            ...
            # Определяем правильное имя файла для архива
...
def update_readme(results, notes):
    ...
    lines.append(f"📊 **Статистика:** {successful} успешно, {failed} ошибок\n")
